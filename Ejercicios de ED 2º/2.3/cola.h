...
using namespace std;
/**
NOMBRE Y APELLIDOS: Abraham Martín Vicente y Jorge López Carrión (D24)
*/
	...
	void llevarAlPrincipio(unsigned int pos) {
		...
		if (pos > _nelems || pos == 0) {
			...
			if (pos != 1) {
				while (num != pos) {
					if (pos != num - 1) {
						anterior = actual;				//nodo anterior
						actual = (*actual)._sig;		//nodo del numero
						num++;							//numero del nodo actual
					}
				}
				if (actual == _ult) {
					_ult = anterior;
