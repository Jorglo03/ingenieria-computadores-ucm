...
public:
	...
	**/
	void minimoACima() {
		...
		if (!esVacia()) {
			Nodo* actual = _cima, * menor = _cima, * ant = _cima, *actual_ant = _cima;
			while (actual->_sig != nullptr) {
				if (actual->_elem < menor->_elem) {
					menor = actual;
					ant = actual_ant;
				}
				actual_ant = actual;
			...
			if (menor->_elem != _cima->_elem) {
				if (menor->_sig == nullptr) {
					ant->_sig = nullptr;
				}
				else {
					ant->_sig = menor->_sig;
