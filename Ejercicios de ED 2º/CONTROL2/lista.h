...
public:
	...
	void ordenar() {
		// OPERACION A IMPLEMENTAR. Nota: la complejidad en el peor caso deberá ser O(n log n).
		// No se permite el uso, directo o indirecto, de operaciones que supongan gestión de memoria dinámica
		// (new, delete), ni tampoco asignar valores a contenidos de nodos: la solución debe realizar
		// únicamente una reorganización de los punteros de la estructura de datos. 
		// Pueden definirse y utilizarse los métodos privados que se consideren oportuno (siempre métodos
		// privados). 
		if(_numElems > 1)
			mergeSort(_prim, _numElems);
	}
...
private:
	...
	*/
	...
	Nodo* mergeSort(Nodo* cabeza, int tam) {
		if (tam == 1) {
			return cabeza;
		}
		else {
			...
			int i = 1;
			while (lento != nullptr && rapido != nullptr && rapido->_sig != nullptr && rapido->_sig->_sig != nullptr && i < tam / 2) {
				lento = lento->_sig;					//mitad
				rapido = rapido->_sig->_sig;
				i++;
			}



			return merge(mergeSort(cabeza, i), i, mergeSort(lento->_sig, tam - i), tam - i);
		}
	}

	Nodo* merge(Nodo* primero, int tam1, Nodo* segundo, int tam2) {
		if (primero == nullptr)
			return segundo;
		else if (segundo == nullptr)
			return primero;
		Nodo* sub_lista = primero;
		Nodo* aux;
		while (tam1 > 0 && tam2 > 0) {
			if (primero->_elem > segundo->_elem) {
				aux = segundo;
				segundo = segundo->_sig;
				insertaPuntero(aux, primero->_ant, primero);				
				if (aux->_sig == sub_lista) {
					sub_lista = aux;
				}				
				tam2--;
			}
			else if (primero->_elem == segundo->_elem) {
				aux = segundo;
				segundo = segundo->_sig;				
				insertaPuntero(aux, primero->_ant, primero);
				primero = primero->_sig;
				if (aux->_sig == sub_lista) {
					sub_lista = aux;
				}
				tam1--;
	...
	void insertaPuntero(Nodo* nodo, Nodo* nodoAnt, Nodo* nodoSig) {
		//Extraccion
		if (nodo->_ant != nullptr && nodo->_sig != nullptr) {
			nodo->_ant->_sig = nodo->_sig;
			nodo->_sig->_ant = nodo->_ant;
		}
		else if(nodo->_ant == nullptr && nodo->_sig != nullptr){
			nodo->_sig->_ant = nullptr;
			_prim = nodo->_sig;
		}
		else if(nodo->_ant != nullptr && nodo->_sig == nullptr){
			nodo->_ant->_sig = nullptr;
			_ult = nodo->_ant;
		}

		//Insercion
		if (nodoAnt == nullptr && nodoSig != nullptr) {
			nodo->_ant = nullptr;
			nodo->_sig = nodoSig;
			nodoSig->_ant = nodo;
			_prim = nodo;
		}
		else if (nodoAnt != nullptr && nodoSig == nullptr) {
			nodo->_sig = nullptr;
			nodo->_ant = nodoAnt;
			nodoAnt->_sig = nodo;
			_ult = nodo;
