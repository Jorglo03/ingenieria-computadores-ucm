...
public:
	...
	*/
	void k_malea(int k) {
		if (k < 3) throw EArgumentoInvalido();
		// A IMPLEMENTAR: A partir de aquí puede suponerse que k>=3. 
		unsigned int contador = 0, num_elems = 0;
		Nodo *nodo1 = _prim, *nodo2 = _prim, *actual = _prim, *nodo_ant2 = _prim, *nodo_sig2 = _prim;
		while (num_elems != _numElems) {
			...
			if (contador == 1) {
				...
				nodo1->_ant->_sig = nodo2;
				nodo1->_sig->_ant = nodo2;
				nodo_ant2 = nodo2->_ant;
				if (nodo2 != _ult)
					nodo_sig2 = nodo2->_sig;
				else
					nodo_sig2 = nullptr;
				nodo2->_ant = nodo1->_ant;
				nodo2->_sig = nodo1->_sig;
				nodo1->_ant = nodo_ant2;
				nodo1->_sig = nodo_sig2;
				if (nodo2 != _ult)
					nodo_sig2->_ant = nodo1;
				else
					_ult = nodo1;
				nodo_ant2->_sig = nodo1;
			}
