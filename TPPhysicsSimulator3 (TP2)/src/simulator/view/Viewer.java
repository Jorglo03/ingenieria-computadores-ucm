...
class Viewer extends SimulationViewer {
	...
	private void initGUI() {
		...
		addKeyListener(new KeyListener() {
			...
			@Override
			public void keyTyped(KeyEvent e) {		//presionar y soltar
			}

			@Override
			public void keyReleased(KeyEvent e) {	//soltar
			}
	...
	private void showHelp(Graphics2D g) {
		...
		int x = 10; // coordenada x de inicio
	    int y = 20; // coordenada y de inicio
	    
	    g.setColor(Color.RED);
	    g.drawString("h: toggle help, v: toggle vectors, +: zoom-in, -: zoom-out, =: fit", x, y);
	    y += 20; // incrementar la coordenada y para la siguiente línea

	    g.drawString("g: show next group", x, y);
	    y += 20;
	    
	    g.drawString("l: move right, j: move left, i: move up, m: move down: k: reset", x, y);
	    y += 20;
	    
	    g.drawString("Scaling ratio: " + _scale, x, y);
	    y += 20;
	    
	    g.setColor(Color.BLUE);
	    if(_selectedGroup == null)
	    	g.drawString("Selected Group: all", x, y);
	    else
	    	g.drawString("Selected Group: " + _selectedGroup, x, y);
	}
	...
	private void drawBodies(Graphics2D g) {
		...
		for (Body b : _bodies) {
			if (isVisible(b)) {
				...
				if (_showVectors) {
					// Normalizar el vector de velocidad para obtener la dirección
				    double velocityDirectionX = b.getVelocity().direction().getX();
				    double velocityDirectionY = -b.getVelocity().direction().getY();

				    // Multiplicar la dirección por un número arbitrario para obtener una longitud adecuada para la flecha
				    int arrowVelocityDirectionX = (int) (velocityDirectionX * 20);
				    int arrowVelocityDirectionY = (int) (velocityDirectionY * 20);

				    drawLineWithArrow(g, x, y, x + arrowVelocityDirectionX, y + arrowVelocityDirectionY, 3, 3, Color.GREEN, Color.GREEN);
				
				    double forceDirectionX = b.getForce().direction().getX();
				    double forceDirectionY = -b.getForce().direction().getY();
				
				    int arrowForceDirectionX = (int) (forceDirectionX * 20);
				    int arrowForceDirectionY = (int) (forceDirectionY * 20);

				    drawLineWithArrow(g, x, y, x + arrowForceDirectionX, y + arrowForceDirectionY, 3, 3, Color.RED, Color.RED);
				...
				// Dibuja el cuerpo como un círculo relleno
